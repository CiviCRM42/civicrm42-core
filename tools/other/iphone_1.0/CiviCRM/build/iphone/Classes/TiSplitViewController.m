...
{
	if (self = [super init]) {
		_civicrmRoot = [rootController retain];
		proxy = split_;
...
-(void)loadView
{
	[[NSNotificationCenter defaultCenter] removeObserver:_civicrmRoot name:UIDeviceOrientationDidChangeNotification object:nil];
	[super loadView];
}
...
{
	[_civicrmRoot refreshOrientationModesIfNeeded:oldCurrentWindow];
}
