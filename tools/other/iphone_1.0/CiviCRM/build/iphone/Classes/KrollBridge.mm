...
-(void)injectPatches
{
	// called to inject any CiviCRM patches in JS before a context is loaded... nice for 
...
{
	// create CiviCRM global object
	NSString *basePath = (url==nil) ? [[NSBundle mainBundle] resourcePath] : [[url path] stringByDeletingLastPathComponent];
	_civicrm = [[CiviCRMObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_civicrm];
	
	NSString *_civicrmNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithUTF8CString([_civicrmNS UTF8String]);
	...
	if (preload!=nil)
	{
		KrollObject *ti = (KrollObject*)[_civicrm valueForKey:@"UI"];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.civicrm.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
